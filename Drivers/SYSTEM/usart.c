...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == USART2_UX)                            /* 如果是串口1，进行串口1 MSP初始化 */
    {
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == USART2_UX)                    /* 如果是串口2 */
    {
        ...
        {
            if (g_usart2_rx_sta & 0x4000)                /* 接收到了0x0d（即回车键） */
            {
                if (g_rx_buffer2[0] != 0x0a)             /* 接收到的不是0x0a（即不是换行键） */
                {
                ...
                else                                    /* 接收到的是0x0a（即换行键） */
                {
                    ...
                    if((g_usart2_rx_sta & 0X3FFF)==0)
                    {
                        g_usart2_rx_sta = 0;           /* 丢弃数据头的0x0d,0x0a(丢弃空数据) */
                    }
                    else
                    {
                        g_usart2_rx_sta |= 0x8000;           /* 接收完成了 */
