...
uint8_t g_rx_buffer2[RXBUFFERSIZE2];  /* HAL库使用的串口接收缓冲 */
...
void usart2_init(uint32_t baudrate)
{
    ...
    HAL_UART_Receive_IT(&g_uart2_handle, (uint8_t *)g_rx_buffer2, RXBUFFERSIZE2); 
}
...
void USART2_UX_IRQHandler(void)
{
    ...
    while (HAL_UART_Receive_IT(&g_uart2_handle, (uint8_t *)g_rx_buffer2, RXBUFFERSIZE2) != HAL_OK)     /* 重新开启中断并接收数据 */
    {
        /* 如果出错会卡死在这里 */
